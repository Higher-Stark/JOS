...
static int
printpurenum(void (*putch)(int, void *), void *putdat, 
	 unsigned long long num, unsigned base)
{
	int count = 0;
	if (num > base) {
		count = printpurenum(putch, putdat, num / base, base);
	}
	 ...
	 unsigned long long num, unsigned base, int width, int padc)
{
	...
	// your code here:
	if (padc == '-') {
		int count = printpurenum(putch, putdat, num, base);
		width -= count;
		while (width-- > 0) 
			putch(' ', putdat);
		return;
...
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap)
{
	...
	// flag to precede with a +/- before number
	int sflag;
	...
	reswitch:
		...
		case 'n': {
				  ...
				  // Your code here
					void *iptr;
					if ((iptr = va_arg(ap, void *)) == NULL) {
						for (int idx = 0; idx != strlen(null_error); idx++)
							putch(null_error[idx], putdat);
					}
					else {
